...
/*   Updated: 2019/04/01 17:17:28 by mlurker          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		fdf_change_color(int keycode, t_field *fdf)
{
	...
	else if (keycode == KEY_W) // глупая попытка сдвинуть цвет в другую сторону
	{
		while (++i < fdf->width * fdf->height)
		{
			if (fdf->points_mem[i].color != DEFAULT_COLOR)
				fdf->points_mem[i].color = get_color(*fdf, i, i + 1);
		}
	}
	fdf_points_copy(fdf);
	fdf_evaluate(fdf);
